...
static std::map<std::string, struct id_entry*> entries;
...
cast_pair (struct sem_rec *left, struct sem_rec *right)
{
	...
	if ((left->s_type & T_DOUBLE) && !(right->s_type & T_DOUBLE))
		R = Builder.CreateSIToFP(R, Type::getDoubleTy(TheContext), "cast");
	else if ((left->s_type & T_INT) && !(right->s_type & T_INT))
...
call(char *f, struct sem_rec *args)
{
    ...
    if (!entry) {
        ...
        if (entries.find(fname) != entries.end()) 
			entry = entries[std::string(fname)];
		else{
    ...
    returnType = TheFunction->getReturnType();

    if(returnType->isDoubleTy())
		result->s_type = T_DOUBLE;
	else if(returnType->isIntegerTy())
        result->s_type = T_INT;

    result->s_value = (void*) callInstance;
    return result;	
}
...
ccand(struct sem_rec *e1, void *m, struct sem_rec *e2)
{
	...
	//printf("sem: ccand not implemented\n");
...
indx(struct sem_rec *x, struct sem_rec *i)
{
	...
	Value *value = Builder.CreateGEP(type, (Value*)x->s_value, makeArrayRef(v));
	return s_node(value, x->s_type * ~T_ARRAY); 
...
{
	BasicBlock *bb = create_named_label(string(id));
	BranchInst *br_inst = (BranchInst*) Builder.GetInsertBlock()->getTerminator();	
	if(br_inst == NULL)
		Builder.CreateBr(bb);
...
struct sem_rec *n()
{
	...
	BasicBlock *bb;
	bb= create_tmp_label();
	Value * bb_val = Builder.CreateBr(bb);
	rec = (node (bb_val, bb, 0, (struct sem_rec *) NULL, (struct sem_rec *) NULL, (struct sem_rec *)NULL) );
	return rec; 
...
op2(const char *op, struct sem_rec *x, struct sem_rec *y)
{
	struct sem_rec *rec;
	Value *left, *right;
	left = (Value *) x->s_value;
	right = (Value*) y->s_value;
	...
	switch(*op){
		case '+':
			if(x->s_type & T_INT)			rec = s_node(Builder.CreateAdd(left, right), x->s_type);
			else if(x->s_type & T_DOUBLE)	rec = s_node(Builder.CreateFAdd(left, right), x->s_type);
			break;
		case '-':
			if(x->s_type & T_INT)			rec = s_node(Builder.CreateSub(left, right), x->s_type);
			else if(x->s_type & T_DOUBLE)   rec = s_node(Builder.CreateFSub(left, right), x->s_type);
			break;
		case '/':
            if(x->s_type & T_INT)           rec = s_node(Builder.CreateSDiv(left, right), x->s_type);
            else if(x->s_type & T_DOUBLE)   rec = s_node(Builder.CreateFDiv(left, right), x->s_type);
            break;
		case '*':
            if(x->s_type & T_INT)           rec = s_node(Builder.CreateMul(left, right), x->s_type);
            else if(x->s_type & T_DOUBLE)   rec = s_node(Builder.CreateFMul(left, right), x->s_type);
            break;
		case '%':
            if(x->s_type & T_INT)           rec = s_node(Builder.CreateSRem(left, right), x->s_type);
            break;
		default:
...
{
	struct sem_rec* rec;
	struct sem_rec* x_cast = x;
	struct sem_rec* y_cast = y;
	...
	switch(*op){
		case '&':
			if(x_cast->s_type & T_INT)
                rec = s_node( Builder.CreateAnd( (Value*)x_cast->s_value, (Value *)y_cast->s_value), x_cast->s_type);
			break;
		case '|':
			if(x_cast->s_type & T_INT)
				rec = s_node( Builder.CreateOr( (Value*)x_cast->s_value, (Value *)y_cast->s_value), x_cast->s_type);
			break;
		case '^':
			if(x_cast->s_type & T_INT)
                rec = s_node( Builder.CreateXor( (Value*)x_cast->s_value, (Value *)y_cast->s_value), x_cast->s_type);
			break;
		case '<':
			if(x_cast->s_type & T_INT)
                rec = s_node( Builder.CreateShl( (Value*)x_cast->s_value, (Value *)y_cast->s_value), x_cast->s_type);
			break;
		case '>':
			if(x_cast->s_type & T_INT)
                rec = s_node( Builder.CreateAShr( (Value*)x_cast->s_value, (Value *)y_cast->s_value), x_cast->s_type);
			break;
		default:
...
rel(const char *op, struct sem_rec *x, struct sem_rec *y)
{
	...
	if(x->s_type & T_INT && y->s_type & T_DOUBLE){
		x_cast = cast(x, T_DOUBLE);
	}
	if(x->s_type & T_DOUBLE && y->s_type & T_INT){
		y_cast = cast(y, T_DOUBLE);
	}


	switch(*op){
		case '<':
			if (x_cast->s_type & T_INT && y_cast->s_type & T_INT)
				val = Builder.CreateICmpSLT((Value *)x_cast->s_value, (Value*)y_cast->s_value);
			else if(x_cast->s_type & T_DOUBLE && y_cast->s_type & T_DOUBLE)
				val = Builder.CreateFCmpOLT((Value *)x_cast->s_value, (Value*)y_cast->s_value);
			break;
		case '>':
			if (x_cast->s_type & T_INT && y_cast->s_type & T_INT)
				val = Builder.CreateICmpSGT( (Value *)x_cast->s_value, (Value*)y_cast->s_value);
			else if(x_cast->s_type & T_DOUBLE && y_cast->s_type & T_DOUBLE)
				val = Builder.CreateFCmpOGT( (Value *)x_cast->s_value, (Value*)y_cast->s_value);
			break;
	}	

	if(strcmp(op, "==") == 0){
		if(x_cast->s_type & T_INT && y_cast->s_type & T_INT){
			val = Builder.CreateICmpEQ( (Value*) x_cast->s_value, (Value*) y_cast->s_value);
		}
		else if(x_cast->s_type & T_DOUBLE && y_cast->s_type & T_DOUBLE){
			val = Builder.CreateFCmpOEQ( (Value*) x_cast->s_value, (Value*) y_cast->s_value);
		}
	}
	else if(strcmp(op, "!=") == 0){
		if(x_cast->s_type & T_INT && y_cast->s_type & T_INT){
			val = Builder.CreateICmpNE( (Value*) x_cast->s_value, (Value*) y_cast->s_value);
		}
		else if(x_cast->s_type & T_DOUBLE && y_cast->s_type & T_DOUBLE){
			val = Builder.CreateFCmpONE( (Value*) x_cast->s_value, (Value*) y_cast->s_value);
		}
	}
	else if(strcmp(op, ">=") == 0){
		if(x_cast->s_type & T_INT && y_cast->s_type & T_INT){
			val = Builder.CreateICmpSGE( (Value*) x_cast->s_value, (Value*) y_cast->s_value);
		}
		else if(x_cast->s_type & T_DOUBLE && y_cast->s_type & T_DOUBLE){
			val = Builder.CreateFCmpOGE( (Value*) x_cast->s_value, (Value*) y_cast->s_value);
		}
	}
	else if(strcmp(op, "<=") == 0){
		if(x_cast->s_type & T_INT && y_cast->s_type & T_INT){
			val = Builder.CreateICmpSLE( (Value*) x_cast->s_value, (Value*) y_cast->s_value);
		}
		else if(x_cast->s_type & T_DOUBLE && y_cast->s_type & T_DOUBLE){
			val = Builder.CreateFCmpOLE( (Value*) x_cast->s_value, (Value*) y_cast->s_value);
		}
	}
...
{
	struct sem_rec *new_y;
	if(t & T_INT){
		Type* type = Type::getInt32Ty(TheContext);
		new_y = s_node(Builder.CreateFPToSI((Value *) y->s_value, type), T_INT);
	}
	else{
		Type* type = Type::getDoubleTy(TheContext);
		new_y = s_node(Builder.CreateSIToFP((Value *) y->s_value, type), T_DOUBLE);
	}
	return new_y;
...
set(const char *op, struct sem_rec *x, struct sem_rec *y)
{
	...
	if(*op == '/' || *op == '+' || *op == '-' || *op == '*' || *op == '%'){
		load = Builder.CreateLoad(type, (Value *)x_cast->s_value);
		rec = op2(op, s_node((Value *)load, x_cast->s_type), y_cast);
	}
	else if(*op == '|' || *op == '&' || *op == '^' || strcmp(op, "<=") == 0 || strcmp(op, ">=") == 0){
		load = Builder.CreateLoad(type, (Value *)x_cast->s_value);
		rec = opb(op, s_node((Value *)load, x_cast->s_type), y_cast);
	}

	if(*op == '\0'){
		ret = s_node ( (Value*) Builder.CreateStore( (Value*) y_cast->s_value, (Value*) x->s_value), x->s_type);
	}
	else{
		ret = s_node ( (Value*) Builder.CreateStore( (Value*) rec->s_value, (Value*) x->s_value), x->s_type);
	}
	return ret;	
}
...
genstring(char *s)
{ 
	struct sem_rec *returnVal;
	returnVal = s_node((void*) Builder.CreateGlobalStringPtr(parse_escape_chars(s)), T_STR);
	return returnVal;
}
